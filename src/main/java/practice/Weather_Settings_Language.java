...
public class Weather_Settings_Language extends Base{
		...
		/* Summary: Purpose of the TC is to check if selected by default language is English, list of languages contains all required languages and if there are selected appropriate languages
		 * Step 1: Go to the bottom of the page and check if selected by default language is English
		 * Expected result: Selected by default language is English
		 * Step 2: Click on the field with languages list next to 'Language'
		 * Expected result: Pop up list with possible languages is presented
		 * Step 3: Check if all language names are the same as expected
		 * Expected result: All language names are the same as expected
		 * Step 4: Close the pop up language list
		 * Expected result: The pop up language list is closed
		 ...
		 * Expected result: The language which is clicked is visible as selected
		 */
